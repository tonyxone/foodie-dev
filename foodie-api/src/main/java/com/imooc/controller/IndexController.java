...
public class IndexController {
    ...
    /*
    * 1.后台运营型系统，一旦轮播图发生更改，就可以删除缓存，然后重置
    * 2.定时重置，比如每天凌晨3点重置
    * 3.每个混播图都有可能是一个广告，每个广告都会有一个过期时间，过期了，再重置
    * */
    ...
    public IMOOCJSONResult cats(){
        ...
        String rootCatStr = redisOperator.get("rootCatStr");

        if(StringUtils.isBlank(rootCatStr)){
            list = categoryService.queryAllRootLevelCat();
            redisOperator.set("rootCatStr",JsonUtils.objectToJson(list));
        }else{
            list = JsonUtils.jsonToList(rootCatStr,Category.class);
        }
