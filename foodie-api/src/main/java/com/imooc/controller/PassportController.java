...
public class PassportController extends BaseController {
    ...
    * */
    private void syncShopcartData(String userId,
        ...
        *                  如果cookie中购物车不为空，此时直接放入redis中
        * 2. redis中有数据，如果cookie中的购物车为空，那么直接把redis的购物车覆盖本地cookie
        *                  如果cookie中购物车不为空，如果cookie中的某个商品在redis中存在，则以cookie为主，删除redis中数据，把cookie中的商品直接覆盖
        * 3.同步到redis中去了以后，覆盖本地cookie购物车数据，保证本地购物车的数据是同步
        * */
        ...
        if(StringUtils.isBlank(shopcartJsonRedis)){
            ...
            //redis不为空，cookie不为空，合并cookie和redis中购物车的商品数据(同意商品测覆盖redis)
            if(StringUtils.isNotBlank(shopcartStrCookie)){
                /*
                ...
                for(ShopcartBO redisShopCart : shopcartListRedis){
                    ...
                    for(ShopcartBO cookieShopCart : shopcartListCookie){
                        ...
                        if(redisSpecId.equals(cookieSpecId)){
                            ...
                            //把cookieshopcart放入待删除列表，用以最后的删除与合并
